...
function Select2ajax(psSelectID, psURL) {
    $.getJSON(psURL, function (data) {
        $(psSelectID).select2({ data: data });
    });
}; //End function select2ajax
function Select2ajaxver2($Select2, psURL, targetExistValue) {
    $.getJSON(psURL, function (data) {
        $Select2.empty().select2({ data: data, placeholder: '' });
        if (targetExistValue != "") { $Select2.val(targetExistValue).trigger("change"); } //end if
    });
}; //End function select2ajax
